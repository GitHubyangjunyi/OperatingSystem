...
unsigned long do_exit(unsigned long code)
{
	...
	for(;ax<4;ax++)
	{	color_printk(RED,BLACK,"do_execve ax = %#018lx\n",ax);
		switch_to(current,p);//局部变量编译时在栈上分配的空间,进程间切换不影响,但是此处的ax是全局变量,需要同步机制,不然结果并不是预想那样,结果是会相互破坏
		color_printk(RED,BLACK,"do_execve ax = %#018lx\n",ax);
	}
	while(1);
}
...
//	init_thread,init_tss
	...
	color_printk(WHITE,BLACK,"(init_task_union.stack + STACK_SIZE / sizeof(unsigned long)):%#018lx\n",(init_task_union.stack + STACK_SIZE / sizeof(unsigned long)));
	color_printk(WHITE,BLACK,"(init_task_union.stack + STACK_SIZE / sizeof(unsigned long)):%#018lx\n",(init_task_union.stack + STACK_SIZE / sizeof(unsigned long)));
	color_printk(WHITE,BLACK,"(init_task_union.stack + STACK_SIZE / sizeof(unsigned long)):%#018lx\n",(init_task_union.stack + STACK_SIZE / sizeof(unsigned long)));
	p = container_of(list_next(&current->list),struct task_struct,list);
