...
inline unsigned char io_in8(unsigned short port)//读8位端口,in指令的目的操作数必须是ax系列,加入mfence内存屏障是为了防止处理器乱序执行导致不可预料的后果
{
	unsigned char ret = 0;
...
inline unsigned int io_in32(unsigned short port)//读32位端口
{
	unsigned int ret = 0;
...
inline void io_out8(unsigned short port,unsigned char value)//写8位端口,out指令源操作数必须是ax系列
{
...
inline void io_out32(unsigned short port,unsigned int value)//写32位端口
{
