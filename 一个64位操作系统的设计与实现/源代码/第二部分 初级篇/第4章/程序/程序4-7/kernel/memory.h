...
#define Virt_To_Phy(addr)	((unsigned long)(addr) - PAGE_OFFSET)//用于将内核层虚拟地址转换成物理地址,但有条件限制,目前只有物理地址的前10MB被映射到线性地址0xffff800000000000处(head.S定义的页表中),也只有这10MB可供该宏函数使用
#define Phy_To_Virt(addr)	((unsigned long *)((unsigned long)(addr) + PAGE_OFFSET))//随着内核功能的完善,可用空间将会越来越大,这一行的宏与上一行相反
